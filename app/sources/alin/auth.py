"""Authentification API AL'in (Keycloak "direct grant"), cf. app/sources/base.py
pour les règles éthiques communes.

Flow en deux étapes : POST `ALIN_AUTH_URL` (login/password → access_token
Keycloak), puis échange via `ALIN_TOKEN_EXCHANGE_URL` contre le `jwt_token`
"maison" AL'in, seul token accepté par l'API métier (`x-gexrt-api-key` est
une valeur publique, cf. https://al-in.fr/info). Une 4xx inattendue lève
directement (jamais de contournement CAPTCHA/MFA) : app/main.py bascule alors
vers le fallback app/sources/alin/browser.py. Mot de passe/token jamais loggés.
"""
...
class AlinAuthClient:
    ...
    Pas de requête "refresh" distincte observée : on ré-authentifie avant
        ...
        for attempt in range(1, _MAX_AUTH_ATTEMPTS + 1):
            ...
            if response.status_code >= 400:
                ...
                raise AlinAuthenticationError(
                    ...
                    "intervention manuelle nécessaire (cf. app/sources/alin/browser.py)."
                )
    ...
    async def _exchange_token(self, keycloak_access_token: str) -> str:
        ...
        Pas de header Authorization (observé) : le token Keycloak est
        transmis dans le corps JSON, pas en Bearer. Retente sur erreurs
        réseau/5xx comme authenticate(), échoue net sur 4xx.
        """
        last_error: Exception | None = None
        for attempt in range(1, _MAX_AUTH_ATTEMPTS + 1):
            try:
                ...
                last_error = exc
                logger.warning(
                    "Échec réseau lors de l'échange de token AL'in (tentative %d/%d): %s",
                    attempt,
                    _MAX_AUTH_ATTEMPTS,
                    type(exc).__name__,
                )
            ...
            if response.status_code >= 500:
                ...
                logger.warning(
                    "Erreur serveur (%d) lors de l'échange de token AL'in (tentative %d/%d).",
                    response.status_code,
                    attempt,
                    _MAX_AUTH_ATTEMPTS,
                )
        ...
        raise AlinAuthenticationError(
            f"Échec de l'échange de token AL'in après {_MAX_AUTH_ATTEMPTS} tentatives "
            f"({type(last_error).__name__ if last_error else 'erreur inconnue'})."
        )
